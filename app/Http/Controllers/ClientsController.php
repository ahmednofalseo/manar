...
class ClientsController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        // المشاريع غير المرتبطة بعميل (لزر ربط مشروع)
        $unlinkedProjects = \App\Models\Project::whereNull('client_id')
            ->orderBy('name')
            ->get(['id', 'name', 'project_number']);

        // بناء سجل النشاط الحقيقي من البيانات
        $activities = collect();
        ...
        $activities->push([
            ...
            'detail' => 'بواسطة: مدير النظام',
            'user' => 'مدير النظام',
            'date' => $client->created_at,
        ]);

        // ترتيب حسب التاريخ (الأحدث أولاً)
        $activities = $activities->sortByDesc('date')->take(50)->values();

        return view('clients.show', compact('client', 'activities', 'unlinkedProjects'));
    }
    ...
    /**
     * ربط مشروع موجود بالعميل
     */
    public function linkProject(Request $request, string $id)
    {
        $client = Client::findOrFail($id);
        ...
        if ($project->client_id && $project->client_id != $client->id) {
            return back()->with('error', 'هذا المشروع مرتبط بعميل آخر. يرجى فك الربط أولاً من صفحة المشروع.');
        }

        $project->update(['client_id' => $client->id]);

        return back()->with('success', 'تم ربط المشروع بالعميل بنجاح');
    }
