...
<div class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6" x-data="clientTabs()">
    ...
    <div x-show="activeTab === 'projects'" class="space-y-4">
        <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('projects.create', ['client_id' => $client->id]) }}" class="px-3 py-1 bg-primary-400/20 hover:bg-primary-500/30 text-primary-400 rounded text-sm">
                    <i class="fas fa-plus ml-1"></i>
                    مشروع جديد
                </a>
                @can('update', $client)
                <button type="button" @click="openLinkProjectModal()" class="px-3 py-1 bg-[#1db8f8]/20 hover:bg-[#1db8f8]/30 text-[#1db8f8] rounded text-sm">
    ...
    <div x-show="activeTab === 'activity'" class="space-y-4">
        ...
        <div class="space-y-3">
            ...
            <div class="flex gap-3">
                ...
                <div class="flex-1 pb-3">
                    <p class="text-white text-sm font-semibold">{{ $activity['title'] }}</p>
                    <p class="text-gray-400 text-xs">{{ $activity['detail'] }}</p>
                    <p class="text-gray-500 text-xs mt-0.5">{{ $activity['date']->format('Y-m-d') }} - {{ $activity['date']->format('h:i') }} {{ $activity['date']->format('A') == 'AM' ? 'ص' : 'م' }}</p>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="text-center py-12">
            ...
            <p class="text-gray-400">ستظهر الأنشطة هنا عند إضافة ملاحظات أو مرفقات أو ربط مشاريع بالعميل</p>
        </div>
        @endif
    </div>
</div>
...
<!-- Link Project Modal -->
<div x-data="{ showLinkModal: false }" x-on:open-link-project-modal.window="showLinkModal = true" x-show="showLinkModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div x-show="showLinkModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 bg-black/70 transition-opacity" @click="showLinkModal = false"></div>
        <div x-show="showLinkModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="relative inline-block w-full max-w-md p-6 my-8 overflow-hidden text-right align-middle transition-all transform bg-[#0f172a] rounded-2xl border border-white/10 shadow-xl">
            <h3 class="text-lg font-bold text-white mb-4">ربط مشروع بالعميل</h3>
            @if(isset($unlinkedProjects) && $unlinkedProjects->count() > 0)
            <form action="{{ route('clients.link-project', $client->id) }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-300 text-sm mb-2">اختر المشروع</label>
                    <select name="project_id" required class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40">
                        <option value="">-- اختر مشروع غير مرتبط --</option>
                        @foreach($unlinkedProjects as $proj)
                        <option value="{{ $proj->id }}">{{ $proj->name }} @if($proj->project_number)({{ $proj->project_number }})@endif</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex gap-3 justify-end">
                    <button type="button" @click="showLinkModal = false" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg">إلغاء</button>
                    <button type="submit" class="px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-lg">ربط</button>
                </div>
            </form>
            @else
            <p class="text-gray-400 mb-4">لا توجد مشاريع غير مرتبطة بعميل. يمكنك إنشاء مشروع جديد أولاً.</p>
            <button type="button" @click="showLinkModal = false" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg">إغلاق</button>
            @endif
        </div>
    </div>
</div>
...
function clientTabs() {
    return {
        ...
        openLinkProjectModal() {
            window.dispatchEvent(new CustomEvent('open-link-project-modal'));
        }
    }
}
</script>
