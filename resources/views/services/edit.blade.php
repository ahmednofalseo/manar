...
@section('title', 'تعديل الخدمة - ' . \App\Helpers\SettingsHelper::systemName())
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-white">تعديل الخدمة: {{ $service->name }}</h1>
    ...
    <!-- Form -->
    <form action="{{ route('services.update', $service) }}" method="POST" class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="select-wrapper">
                        ...
                        <select name="category_id" class="w-full bg-[#173343]/90 border-2 border-white/30 rounded-lg px-4 py-3 text-white font-semibold focus:outline-none focus:ring-2 focus:ring-[#1db8f8] focus:border-[#1db8f8] transition-all duration-200" style="background-color: rgba(23, 51, 67, 0.9); color: #ffffff; font-weight: 600; max-width: 100%;">
                            ...
                            <option value="{{ $category->id }}" {{ old('category_id', $service->category_id) == $category->id ? 'selected' : '' }} style="background-color: #173343; color: #ffffff; font-weight: 600; padding: 12px;">
                                {{ $category->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="select-wrapper">
                        ...
                        <select name="parent_id" class="w-full bg-[#173343]/90 border-2 border-white/30 rounded-lg px-4 py-3 text-white font-semibold focus:outline-none focus:ring-2 focus:ring-[#1db8f8] focus:border-[#1db8f8] transition-all duration-200" style="background-color: rgba(23, 51, 67, 0.9); color: #ffffff; font-weight: 600; max-width: 100%;">
                            ...
                            <option value="{{ $parent->id }}" {{ old('parent_id', $service->parent_id) == $parent->id ? 'selected' : '' }} style="background-color: #173343; color: #ffffff; font-weight: 600; padding: 12px;">
                                {{ $parent->name }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input 
                            ...
                            name="is_custom" 
                            value="1"
                            {{ old('is_custom', $service->is_custom) ? 'checked' : '' }}
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input 
                            ...
                            value="1"
                            {{ old('has_sub_services', $service->has_sub_services) ? 'checked' : '' }}
                            class="w-5 h-5 rounded border-white/10 bg-white/5 text-primary-500 focus:ring-primary-400/40"
                        >
