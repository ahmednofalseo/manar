...
<div class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-white">{{ __('Stages') }}</h2>
        @can('update', $project)
    ...
    <div class="space-y-6">
        @foreach($project->projectStages as $index => $stage)
            @php
                $isLast = $loop->last;
                $config = $stageStatusConfigKeys[$stage->status] ?? $stageStatusConfigKeys['جديد'];
            @endphp
                ...
                <div class="flex-1 pb-6">
                    <div class="bg-white/5 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="text-white font-semibold">{{ $stageLabelMap[$stage->stage_name] ?? $stage->stage_name }}</h3>
                            <span class="{{ $config['badge'] }} px-2 py-1 rounded text-xs">{{ $stageStatusLabels[$stage->status] ?? $stage->status }}</span>
                        </div>

                        @if($stage->start_date)
                        <p class="text-gray-400 text-sm mb-2">{{ __('Start Date') }}: {{ $stage->start_date->format('Y-m-d') }}</p>
                        @endif

                        @if($stage->end_date)
                        <p class="text-gray-400 text-sm mb-2">
                            @if($stage->status === 'مكتمل')
                                {{ __('End Date') }}: {{ $stage->end_date->format('Y-m-d') }}
                            @else
                                {{ __('Expected end date') }}: {{ $stage->end_date->format('Y-m-d') }}
                            @endif
                        </p>
                        @endif
                        ...
                        <div class="mt-4 pt-4 border-t border-white/10">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center gap-2 text-sm text-gray-400">
                                    ...
                                    <span>{{ __('Tasks in stage count', ['count' => $stage->tasks->count()]) }}</span>
                                </div>
                            </div>
                            <div class="space-y-2">
                                @foreach($stage->tasks as $task)
                                    @php
                                        $taskStatusMap = [
                                            'new' => ['text' => __('Task status new'), 'class' => 'bg-gray-500/20 text-gray-400', 'icon' => 'circle'],
                                            'in_progress' => ['text' => __('Task status in progress'), 'class' => 'bg-primary-400/20 text-primary-400', 'icon' => 'clock'],
                                            'done' => ['text' => __('Task status done'), 'class' => 'bg-green-500/20 text-green-400', 'icon' => 'check'],
                                            'rejected' => ['text' => __('Task status rejected'), 'class' => 'bg-red-500/20 text-red-400', 'icon' => 'xmark'],
                                        ];
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2 mb-1">
                                                    ...
                                                    <h4 class="text-white text-sm font-semibold group-hover:text-primary-400 transition-colors truncate">{{ $task->display_title }}</h4>
                                                </div>
                                                @if($task->assignee)
                                                ...
                                                <p class="text-gray-400 text-xs flex items-center gap-1 mt-1">
                                                    ...
                                                    @if($task->due_date->isPast() && !in_array($task->status, ['done', 'rejected']))
                                                        <span class="text-red-400">({{ __('Overdue') }})</span>
                                                    @endif
                                                </p>
                                                @endif
                                            </div>
    ...
    <div class="text-center py-12">
        ...
        <h3 class="text-xl font-bold text-white mb-2">{{ __('No stages yet') }}</h3>
        <p class="text-gray-400 mb-4">{{ __('Add stages from edit hint') }}</p>
...
<div id="stageModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center" onclick="closeStageModal(event)">
    <div class="glass-card rounded-xl md:rounded-2xl p-6 max-w-md w-full mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-white">{{ __('Update stage') }}</h3>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="stage_name" required class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40">
                        ...
                        @foreach($project->projectStages as $stage)
                            <option value="{{ $stage->stage_name }}">{{ $stageLabelMap[$stage->stage_name] ?? $stage->stage_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40">
                        <option value="جديد">{{ __('Project stage status new') }}</option>
                        <option value="جارٍ">{{ __('Project stage status running') }}</option>
                        <option value="مكتمل">{{ __('Project stage status completed') }}</option>
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-gray-300 text-sm mb-2">{{ __('Progress percent label') }}</label>
                ...
                <div>
                    <label class="block text-gray-300 text-sm mb-2">{{ __('Internal Notes') }}</label>
