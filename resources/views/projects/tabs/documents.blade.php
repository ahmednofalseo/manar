@php
    ...
    $techStatusLabels = [
        'draft' => __('Document status draft'),
        'submitted' => __('Document status submitted'),
        'approved' => __('Document status approved'),
        'rejected' => __('Document status rejected'),
    ];
    $quoteStatusLabels = [
        'draft' => __('Document status draft'),
        'sent' => __('Quotation status sent'),
        'accepted' => __('Quotation status accepted'),
        'rejected' => __('Document status rejected'),
        'expired' => __('Quotation status expired'),
    ];
@endphp

<div x-data="documentsTab()">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 md:mb-6">
        <h2 class="text-xl md:text-2xl font-bold text-white">{{ __('Project documents title') }}</h2>
    ...
    <div x-show="activeType === 'technical_report'">
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                                    ...
                                    @php
                                        ...
                                        $label = $techStatusLabels[$document->status] ?? $document->status;
                                    @endphp
        ...
        <div class="glass-card rounded-xl md:rounded-2xl p-12 text-center">
            ...
            <p class="text-gray-400 text-lg mb-4">{{ __('No technical reports') }}</p>
            @can('create', \App\Models\Document::class)
    ...
    <div x-show="activeType === 'quotation'">
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                                    ...
                                    @php
                                        ...
                                        $color = $statusColors[$document->status] ?? 'blue';
                                        $label = $quoteStatusLabels[$document->status] ?? $document->status;
                                    @endphp
                                    <span class="bg-{{ $color }}-500/20 text-{{ $color }}-400 px-2 py-0.5 rounded text-xs">{{ $label }}</span>
                                </h3>
                                <p class="text-gray-400 text-sm mt-1">
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-400 mt-1">
                                    ...
                                    <span class="text-[#1db8f8] font-semibold">
                                        ...
                                        {{ number_format($document->total_price, 2) }} {{ __('Currency SAR') }}
                                    </span>
                                    @endif
                                    <span>
                                        <i class="fas fa-calendar ml-1"></i>
        ...
        <div class="glass-card rounded-xl md:rounded-2xl p-12 text-center">
            ...
            <p class="text-gray-400 text-lg mb-4">{{ __('No quotations') }}</p>
            @can('create', \App\Models\Document::class)
