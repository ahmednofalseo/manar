<div class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-white">{{ __('Third party') }} ({{ $project->thirdParties->count() }})</h2>
        @can('update', $project)
        <button type="button" onclick="openThirdPartyModal()" class="px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-lg text-sm">
        ...
        <table class="w-full">
            <thead>
                <tr class="text-right border-b border-white/10">
                    <th class="text-gray-400 text-sm font-normal pb-3">{{ __('Entity name') }}</th>
                    <th class="text-gray-400 text-sm font-normal pb-3">{{ __('Date') }}</th>
                    <th class="text-gray-400 text-sm font-normal pb-3">{{ __('Note') }}</th>
            ...
            <tbody>
                @foreach($project->thirdParties as $thirdParty)
                    <tr class="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td class="py-3">
                            @can('update', $project)
                            <form action="{{ route('projects.thirdparty.destroy', [$project->id, $thirdParty->id]) }}" method="POST" class="inline" onsubmit="return confirm({{ json_encode(__('Confirm delete third party')) }})">
                                @csrf
                                @method('DELETE')
    ...
    <div class="text-center py-12">
        ...
        <h3 class="text-xl font-bold text-white mb-2">{{ __('No third parties') }}</h3>
        <p class="text-gray-400 mb-4">{{ __('Add third parties hint') }}</p>
    </div>
    @endif
</div>
...
<div id="thirdPartyModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center" onclick="closeThirdPartyModal(event)">
    <div class="glass-card rounded-xl md:rounded-2xl p-6 max-w-md w-full mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-white">{{ __('Add third party') }}</h3>
            <button type="button" onclick="closeThirdPartyModal()" class="text-gray-400 hover:text-white">
        ...
        <form action="{{ route('projects.thirdparty.store', $project->id) }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-gray-300 text-sm mb-2">{{ __('Entity name') }} <span class="text-red-400">*</span></label>
                    <input type="text" name="name" required class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40" placeholder="{{ __('Third party name placeholder') }}">
                </div>
                ...
                <div>
                    <label class="block text-gray-300 text-sm mb-2">{{ __('Note') }}</label>
                    <textarea name="notes" rows="3" class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40" placeholder="{{ __('Third party notes placeholder') }}"></textarea>
                </div>
            </div>
