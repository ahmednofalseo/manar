...
@section('title', 'تعديل المرحلة - ' . \App\Helpers\SettingsHelper::systemName())
...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl md:text-3xl font-bold text-white">تعديل المرحلة</h1>
    <a href="{{ route('settings.project-stages.index') }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg transition-all duration-200">
...
<!-- Form -->
    <form method="POST" action="{{ route('settings.project-stages.update', $projectStage) }}" class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    @csrf
    @method('PUT')

    <div class="space-y-6">
        <div>
            <label class="block text-gray-300 text-sm mb-2">اسم المرحلة <span class="text-red-400">*</span></label>
            <input 
                type="text" 
                name="name" 
                value="{{ old('name', $projectStage->name) }}"
                required
        ...
        <div>
            ...
            <input 
                type="text" 
                name="name_en" 
                value="{{ old('name_en', $projectStage->name_en) }}"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
            >
        </div>
        ...
        <div>
            ...
            <textarea 
                ...
                rows="3"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
            >{{ old('description', $projectStage->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input 
                    type="text" 
                    name="icon" 
                    value="{{ old('icon', $projectStage->icon) }}"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                    placeholder="مثال: fas fa-building"
                >
                <p class="text-gray-400 text-xs mt-1">استخدم أيقونات Font Awesome</p>
            </div>

            <div>
                <label class="block text-gray-300 text-sm mb-2">اللون (Hex Code)</label>
                <input 
                    type="text" 
                    name="color" 
                    value="{{ old('color', $projectStage->color) }}"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                    placeholder="مثال: #1db8f8"
                >
                <p class="text-gray-400 text-xs mt-1">استخدم كود اللون Hex</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input 
                    type="number" 
                    name="order" 
                    value="{{ old('order', $projectStage->order) }}"
                    min="0"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                >
            </div>
            ...
            <div>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input 
                        ...
                        name="is_active" 
                        value="1"
                        {{ old('is_active', $projectStage->is_active) ? 'checked' : '' }}
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            <a href="{{ route('settings.project-stages.index') }}" class="px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-lg transition-all duration-200">
                إلغاء
            </a>
