...
@section('title', __('Add job title') . ' - ' . \App\Helpers\SettingsHelper::systemName())
@section('page-title', __('Add job title'))
...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl md:text-3xl font-bold text-white">{{ __('Add job title') }}</h1>
...
<form method="POST" action="{{ route('settings.job-titles.store') }}" class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    @csrf
    <div class="space-y-6">
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required maxlength="255"
                ...
                placeholder="{{ __('Job title Arabic placeholder') }}">
            @error('name')
        ...
        <div>
            ...
            <input type="text" name="name_en" value="{{ old('name_en') }}" maxlength="255"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                placeholder="{{ __('Job title English placeholder') }}">
        </div>
