...
@section('title', 'إضافة نوع مشروع جديد - ' . \App\Helpers\SettingsHelper::systemName())
@section('page-title', 'إضافة نوع مشروع جديد')
...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl md:text-3xl font-bold text-white">إضافة نوع مشروع جديد</h1>
    <a href="{{ route('settings.project-types.index') }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg transition-all duration-200">
...
<!-- Form -->
    <form method="POST" action="{{ route('settings.project-types.store') }}" class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    @csrf

    <div class="space-y-6">
        <div>
            <label class="block text-gray-300 text-sm mb-2">اسم النوع <span class="text-red-400">*</span></label>
            <input 
                type="text" 
                name="name" 
                value="{{ old('name') }}"
        ...
        <div>
            ...
            <input 
                ...
                value="{{ old('name_en') }}"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                placeholder="Example: Design & Supervision"
            >
        </div>
        ...
        <div>
            ...
            <textarea 
                ...
                rows="3"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                placeholder="وصف مختصر عن نوع المشروع"
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            <a href="{{ route('settings.project-types.index') }}" class="px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-lg transition-all duration-200">
                إلغاء
            </a>
