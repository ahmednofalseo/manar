...
@section('title', 'تعديل نوع المشروع - ' . \App\Helpers\SettingsHelper::systemName())
@section('page-title', 'تعديل نوع المشروع')
...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl md:text-3xl font-bold text-white">تعديل نوع المشروع</h1>
...
<!-- Form -->
    <form method="POST" action="{{ route('settings.project-types.update', $projectType) }}" class="glass-card rounded-xl md:rounded-2xl p-4 md:p-6">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <input 
                type="text" 
                name="name_en" 
                value="{{ old('name_en', $projectType->name_en) }}"
                class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
            >
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input 
                    type="number" 
                    name="order" 
                    value="{{ old('order', $projectType->order) }}"
                    min="0"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/40"
                >
            </div>
