...
@section('title', 'تعديل قالب مسار - ' . \App\Helpers\SettingsHelper::systemName())
@section('page-title', 'تعديل قالب مسار')
...
<div x-data="workflowBuilder()" class="max-w-6xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-white">تعديل قالب مسار: {{ $workflowTemplate->name }}</h1>
    ...
    <!-- Form -->
    <form action="{{ route('workflow-templates.update', $workflowTemplate) }}" method="POST" @submit.prevent="submitForm">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="select-wrapper">
                        ...
                        >
                            ...
                            <option value="{{ $service->id }}" {{ old('service_id', $workflowTemplate->service_id) == $service->id ? 'selected' : '' }} style="background-color: #173343; color: #ffffff; font-weight: 600; padding: 12px;">
                                {{ $service->name }}
                            </option>
                            @endforeach
                        </select>
            ...
            <template x-for="(step, index) in steps" :key="index">
                <div>
                    ...
                    @if(isset($workflowTemplate->steps[$index]))
                    <input type="hidden" :name="`steps[${index}][id]`" value="{{ $workflowTemplate->steps[$index]->id }}">
                    @endif
                </div>
            </template>
...
function workflowBuilder() {
    return {
        serviceId: '{{ old("service_id", $workflowTemplate->service_id ?? "") }}',
        templateName: '{{ old("name", $workflowTemplate->name) }}',
        templateDescription: '{{ old("description", $workflowTemplate->description ?? "") }}',
        isDefault: {{ old("is_default", $workflowTemplate->is_default) ? 'true' : 'false' }},
        isActive: {{ old("is_active", $workflowTemplate->is_active) ? 'true' : 'false' }},
        steps: @json(old('steps', $workflowTemplate->steps->map(function($step) {
            ...
            return [
                ...
                'duration' => $step->default_duration_days,
                'order' => $step->order,
                'is_parallel' => $step->is_parallel,
                'is_required' => $step->is_required,
                'requires_approval' => $outputs['requires_approval'] ?? false,
                'requires_files' => $outputs['requires_files'] ?? false,
            ];
        }))),
        loading: false,
